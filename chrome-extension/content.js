...
function extractLinkedInData() {
    ...
    try {
        ...
        // Alternative method for company
        if (!data.company) {
            const subtitleElement = document.querySelector('.text-body-medium.break-words');
            if (subtitleElement) {
                const text = subtitleElement.textContent.trim();
                // Extract company from "Title at Company" format
        ...
        if (!data.title) {
            ...
            if (titleElement) {
                data.title = titleElement.textContent.trim().split(' at ')[0];
            }
        }
